...
-- ProgrammingEventNotification 핸들러 (외출방범/재택안심)
local programming_event_handler_sds = function(driver, device, zb_rx)
  ...
  if event_code == nil then
    ...
    if bytes and #bytes >= 2 then
      event_code = bytes:byte(2)
    end
  end

  log.info("[Security] ProgramEventCode: " .. tostring(event_code))

  if event_code == 10 then
    log.info("[Security] 외출방범 설정됨")
  elseif event_code == 11 then
    log.info("[Security] 재택안심 설정됨")
  elseif event_code == 12 then
    log.info("[Security] 방범 해제됨")
  end
end
